...
resource null_resource setup_argocd {
  ...
  provisioner "local-exec" {
    command = "${path.module}/scripts/setup-argocd.sh '${var.config_repo}' '${var.config_paths.infrastructure}' '${local.config_project}' '${var.application_repo}' '${var.application_paths.infrastructure}/namespaces' '${var.namespace}' '${local.application_branch}'"
